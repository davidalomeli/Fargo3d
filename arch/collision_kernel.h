...
#else
      if ( p > o )  m[p+o*NFLUIDS] = -dt*omega*alpha[p+o*NFLUIDS]*0.5*(rho[0][l] + rho[0][idm])*rho_p/rho_o;
      else m[p+o*NFLUIDS] = -dt*omega*alpha[p+o*NFLUIDS]*0.5*(rho[0][l] + rho[0][idm]);
#endif
#else
...
#else
        if( q > p ) sum += alpha[q+p*NFLUIDS]*0.5*(rho[0][l] + rho[0][idm])*rho_q/rho_p;
	  else        sum += alpha[q+p*NFLUIDS]*0.5*(rho[0][l] + rho[0][idm]);
#endif
#else
